...
interface PopulationContextType {
	...
	dailyPopulationUpdate: (yang: number) => void;
...
export function PopulationProvider({ children }: { children: ReactNode }) {
	...
	const dailyPopulationUpdate = useCallback((yang: number) => {
		setPopulationState(prev => {
			...
			if (yang > 50) {
				const maxDecrease = 5 + Math.min(20, yang / 73);
				const decreasePercent = maxDecrease / 100;
				newPop -= prev * decreasePercent;
			}
