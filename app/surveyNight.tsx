...
export default function SurveyNightPage() {
	...
	useEffect(() => {
		...
		if (saved && saved.savedAt === todayStr) {
			...
			if (saved.triviaOptions && typeof saved.triviaOptions.randomTriviaIndex === 'number') setRandomTriviaIndex(saved.triviaOptions.randomTriviaIndex);
		}
	...
	// Build shuffled trivia options when trivia index changes
	useEffect(() => {
		if (randomTriviaIndex !== null && TRIVIA[randomTriviaIndex]) {
			...
			setTriviaOptions({ answers, correctLocalIndex });
			// if we had saved progress, set local correct index for evaluation
